...
these are simply instructions to help guide OnshapeGPT through the template. 

MOST IMPORTANT DIRECTION!!!!!!!
DO NOT include any text that is not code in your responses.
...
def convert_link(old_link):
    ...
    if "cad.onshape.com/documents/" in old_link:
        ...
        did_link = f"https://cad.onshape.com/api/partstudios/d/{doc_id}/"
...
- parameterIDs: center and radius
  - center: Vector. A 3D length vector in world space. Default value: vector(0,0,0)
  - radius: Vector. The three (equal) radii, as measured along the x, y, and z axes. Default value: vector(1,1,1)

Cylinder:
- featureType: fCylinder 
- parameterIDs: topCenter, bottomCenter, and radius
  - topCenter: Vector. A 3D length vector in world space. Default value: vector(1,1,1)
  - bottomCenter: Vector. A 3D length vector in world space. Default value: vector(1,1,0)
...
if api_url == "Invalid Link Format":
  ...
  response = requests.post(did_link+'versions', headers=headers, auth=os_api_keys, json={"name":"*Replace this with the text of the user's prompt. Add (V2) at the end."})
  #print(response)
